    ...
    *state.stop_flag.lock() = Some(stop_flag);
    ...
    thread::spawn(move || {
        ...
        let config = match device.default_input_config() {
            Ok(c) => c,
            Err(e) => {
                info!("[audio] default_input_config failed: {}", e);
                return;
            }
        };
        ...
        *sample_rate_arc.lock() = sr;
        ...
        let stream = match stream {
            Ok(s) => s,
            Err(e) => {
                info!("[audio] build_input_stream failed: {}", e);
                return;
            }
        };
...
async fn speak(text: String, app: AppHandle) -> Result<(), String> {
    *app.state::<AppState>().status.lock() = "speaking".to_string();
    ...
    if !http_status.is_success() {
        ...
        info!(
            "[tts] Noiz failed — falling back to say. body: {}",
            body.chars().take(300).collect::<String>()
        );
        say_fallback(&text);
    } else {
        ...
        if audio_bytes.len() < 44 || !audio_bytes.starts_with(b"RIFF") {
            ...
            fs::write(&cache_path, &audio_bytes)
